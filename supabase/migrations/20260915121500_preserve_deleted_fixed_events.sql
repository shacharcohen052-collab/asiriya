-- Respect deliberate soft deletion of recurring events.
-- ON CONFLICT DO NOTHING leaves an inactive event inactive after refresh.
...
DECLARE
  ...
  v_day INTEGER;
BEGIN
  IF NOT public.is_approved_member() THEN RAISE EXCEPTION 'not authorized'; END IF;
  IF p_end_date < p_start_date OR p_end_date - p_start_date > 366 THEN RAISE EXCEPTION 'invalid date range'; END IF;
  ...
  WHILE v_date <= p_end_date LOOP
    ...
    VALUES ('זום PT100',v_date,'11:45','12:00',true,'fixed_schedule',true,true,1,public.current_profile_id(),true,
    ...
    IF v_day <> 5 THEN
      INSERT INTO public.schedule_events
        (title,event_date,start_time,end_time,is_fixed,source,allows_attendance_plan,counts_for_score,score_value,created_by,is_active,dedupe_key)
      VALUES ('זום PT100',v_date,'18:00','18:30',true,'fixed_schedule',true,true,1,public.current_profile_id(),true,
        lower('זום PT100')||'|'||v_date::text||'|18:00:00|18:30:00')
      ON CONFLICT (dedupe_key) DO NOTHING;
    ELSE
      UPDATE public.schedule_events SET is_active=false
      WHERE event_date=v_date AND start_time='18:00' AND end_time='18:30'
        AND is_fixed=true AND source='fixed_schedule';
    END IF;
